...
var CryptoJS = CryptoJS || function(s, p) {
    var m = {},
        ...
        a = v.Latin1 = {
            ...
            parse: function(b) { for (var a = b.length, g = [], j = 0; j < a; j++) g[j >>> 2] |= (b.charCodeAt(j) & 255) << 24 - 8 * (j % 4); return new q.init(g, a) }
        },
...
class Dialog {
    ...
    show(theme = "warning", backdrop = true) {
        ...
        // $('#'+this.cloneId+' .modal-bg').addClass(this.class);
        $('#' + this.cloneId + ' .modal-title').html(this.title);
        $('#' + this.cloneId + ' .modal-subtitle').html(this.subtitle);
        ...
        if (this.options.framework == "bootstrap") {
            ...
            //     // backdrop: true
            // });
            var myModal = bootstrap.Modal.getOrCreateInstance('#'+this.cloneId);
            myModal.show();
